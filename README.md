
## 📋 Описание

Этот скрипт автоматизирует процесс создания новых пользователей на Linux серверах, избавляя от необходимости вручную выполнять повторяющиеся команды при развертывании новых виртуалок.

### ✨ Возможности

- ✅ Создание пользователя с указанием имени
- ✅ Установка безопасного пароля
- ✅ Автоматическое создание `.ssh` директории
- ✅ Установка правильных прав доступа
- ✅ Поддержка всех основных дистрибутивов Linux

## 🛠 Установка и использование

### Способ 1: Запуск с GitHub (Рекомендуемый)

**Одна команда для запуска на удаленном сервере:**

bash

```
ssh пользователь@IP_СЕРВЕРА "curl -s https://raw.githubusercontent.com/ВАШ_USERNAME/scripts/main/setup_user.sh -o /tmp/setup_user.sh && chmod +x /tmp/setup_user.sh && /tmp/setup_user.sh"
```

**Примеры для разных облачных провайдеров:**

bash

```
# Для AWS EC2 (пользователь ubuntu)
ssh ubuntu@12.34.56.78 "curl -s https://raw.githubusercontent.com/yourname/scripts/main/setup_user.sh -o /tmp/setup_user.sh && chmod +x /tmp/setup_user.sh && /tmp/setup_user.sh"

# Для DigitalOcean (пользователь root)
ssh root@12.34.56.78 "curl -s https://raw.githubusercontent.com/yourname/scripts/main/setup_user.sh -o /tmp/setup_user.sh && chmod +x /tmp/setup_user.sh && /tmp/setup_user.sh"

# Для VPS с пользователем admin
ssh admin@12.34.56.78 "curl -s https://raw.githubusercontent.com/yourname/scripts/main/setup_user.sh -o /tmp/setup_user.sh && chmod +x /tmp/setup_user.sh && /tmp/setup_user.sh"
```

### Способ 2: Локальный запуск

Если скрипт уже на сервере:

bash

```
# Скачать и сделать исполняемым
curl -s https://raw.githubusercontent.com/yourname/scripts/main/setup_user.sh -o setup_user.sh
chmod +x setup_user.sh

# Запустить
./setup_user.sh
```

### Способ 3: Прямое выполнение через curl

bash

```
curl -s https://raw.githubusercontent.com/yourname/scripts/main/setup_user.sh | bash
```

## 🎯 Что делает скрипт

1. **Запрашивает имя пользователя**
2. **Создает системного пользователя:**
	bash
	```
	sudo adduser --gecos "" --disabled-password username
	```
3. **Устанавливает пароль** (с безопасным вводом)
4. **Создает SSH структуру:**
	bash
	```
	mkdir -p ~/.ssh
	chmod 700 ~/.ssh
	```
5. **Выводит информацию** о созданном пользователе

## 📝 Пример работы

bash

```
$ ./setup_user.sh

=== Автоматическое создание пользователя ===
Введите имя пользователя: developer

Установите пароль для пользователя developer:
New password: ********
Retype new password: ********

=== Готово ===
Пользователь: developer
Переключение: su - developer
SSH директория: /home/developer/.ssh
```

## 🐛 Поддерживаемые системы

- ✅ Ubuntu 18.04+
- ✅ Debian 9+
- ✅ CentOS 7+
- ✅ Amazon Linux 2
- ✅ И другие дистрибутивы на базе Linux

## 🔧 Требования

- Доступ к серверу по SSH
- Права sudo для создания пользователей
- Доступ в интернет (для скачивания скрипта)

## 📁 Структура проекта

text

```
scripts/
├── README.md          # Эта документация
├── setup_user.sh      # Основной скрипт
└── LICENSE           # Лицензия (опционально)
```

## 🤝 Разработка

Чтобы внести изменения в скрипт:

1. Клонируйте репозиторий:

bash

```
git clone https://github.com/yourname/scripts.git
```

1. Внесите изменения в `setup_user.sh`
2. Закоммитьте и запушите:

bash

```
git add setup_user.sh
git commit -m "Описание изменений"
git push origin main
```

## ⚠️ Примечания по безопасности

- 🔐 Пароль вводится безопасно (не отображается на экране)
- 📁 Директория `.ssh` создается с правильными правами (700)
- 👥 Скрипт требует sudo права для создания пользователей
